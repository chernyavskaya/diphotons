...
//	TString subcategory = "*((MVAOutputTransformed>0.25)&&(MVAOutputTransformed<0.44))";
...
//	double minevents = 4; //for bkg  # for MVA :100,  because still need to be able to split in MX
	...
	TString date = "15_10_2019";
	TString s; 
	TString outname = s.Format("output_SB_%s_cat4%s_minevents%.0f",what_to_opt.Data(),outstr.Data(),minevents);
...
int minevt_cond_n[10] = {};
	...
	do {
		max_n[0]=0;
		sig_n[0] = hist_S->Integral(1,hist_S->FindBin(start_n[0]));
		bkg_n[0] = hist_B->Integral(1,hist_B->FindBin(start_n[0]));
		if (bkg_n[0]!=0) max_n[0]=pow(sig_n[0],2)/bkg_n[0];
		start_n[1]=start_n[0]+precision;
		do {
			max_n[1]=0;
			sig_n[1] = hist_S->Integral(hist_S->FindBin(start_n[0])+1,hist_S->FindBin(start_n[1]));
			bkg_n[1] = hist_B->Integral(hist_B->FindBin(start_n[0])+1,hist_B->FindBin(start_n[1]));
			if (bkg_n[1]!=0) max_n[1]=pow(sig_n[1],2)/bkg_n[1];
			start_n[2]=start_n[1]+precision;
			do{
				max_n[2]=0;
				sig_n[2] = hist_S->Integral(hist_S->FindBin(start_n[1])+1,hist_S->FindBin(start_n[2]));
				bkg_n[2] = hist_B->Integral(hist_B->FindBin(start_n[1])+1,hist_B->FindBin(start_n[2]));
				...
				do{
					...
					if (NCAT<=3) {
						...
						sig_n[3] = hist_S->Integral(hist_S->FindBin(start_n[2])+1,hist_S->FindBin(start_n[3]));
						bkg_n[3] = hist_B->Integral(hist_B->FindBin(start_n[2])+1,hist_B->FindBin(start_n[3]));
					}
               ...
               if (NCAT<=4) {
                  ...
                  bkg_n[4] = 1.;
						...
						sig_n[4] = hist_S->Integral(hist_S->FindBin(start_n[3])+1,hist_S->FindBin(END));
						bkg_n[4] = hist_B->Integral(hist_B->FindBin(start_n[3])+1,hist_B->FindBin(END));
               }
					...
					for (int index=0;index<NCAT;index++){
						max_sum+=max_n[index];
						minevt_cond_n[index] = (bkg_n[index]>minevents);
					}
