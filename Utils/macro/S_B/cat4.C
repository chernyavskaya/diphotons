...
int main(int argc, char* argv[]){
	...
	double bkg_yields[10] = {};
	double sig_yields[10] = {};
...
max=0;
	...
	//	cout<<max1<<endl;
		...
		//	cout<<max2<<endl;
			...
			//	cout<<max3<<endl;
					...
					if (((max1+max2+max3+max4)>=max) && (b1>minevents) && (b2>minevents)  && (b3>minevents) && (b4>minevents) ) {
	...
	for (int index=0;index<NCAT;index++){
		bkg_yields[index] = hist_B->Integral(hist_B->FindBin(borders[index]),hist_B->FindBin(borders[index+1])); 
		sig_yields[index] = hist_S->Integral(hist_S->FindBin(borders[index]),hist_S->FindBin(borders[index+1]));
	}
	...
	out<<endl;
	out<<"sqrt(S**2/B) in each category : "<<sqrt(max1_final)<<"   "<<sqrt(max2_final)<<"   " << sqrt(max3_final)<<"   " << sqrt(max4_final) <<"    , max = "<<sqrt(max)<<endl;
	out<<endl;
