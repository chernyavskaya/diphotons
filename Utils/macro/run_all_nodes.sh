...
for node in jsons_hh_SM/node_*.json; do name=$(basename $node | sed 's%.json%%'); 
echo "------------------------------------------STARTING NODE -----------------------------------------------------"
echo $name
echo "-------------------------------------------------------------------------------------------------------------"
...
## MVA boundaries optimization with fixed cut in MX and Mjj
    ./categoryOptimizationMultiDim.py --load jsons_hh_SM/optimize_hh_MVA.json --load $node --infile ${infile} --sigfile ${infile}  --label test_SM -o outPlots/out_${now}_${name}_transformed_MVA/

## MX boundaries optimization with optimized MVA boundaries and fixed cut in Mjj
#    ./categoryOptimizationMultiDim.py --load jsons_hh_SM/optimize_hh_MX.json --load $node --infile ${infile} --sigfile ${infile}  --label test_SM -o outPlots/out_${now}_${name}_transformed_MX/



## MVA boundaries optimization with optimized MX boundaries and fixed cut in Mjj
#    ./categoryOptimizationMultiDim.py --load jsons_hh_SM/optimize_hh_MVA_MXboundaries.json --load $node --infile ${infile} --sigfile ${infile}  --label test_SM -o outPlots/out_${now}_${name}_transformed_MVA_step2/
